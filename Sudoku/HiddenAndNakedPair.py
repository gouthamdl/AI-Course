...
def checkHiddenPairs(grid,unit):
    ...
    global gridmap
    # Maps Each pairs of number ina domain to their counts across a unit
    pairmap = defaultdict(int)
    # Maps each pair of numbers in a domain to the cells which contain it
    cellmap = defaultdict(list)
    # Keeps a count of how many times a number has been seen
...
#        domain = variable.getdomain()
    ...
    for value in domain:
        ...
        if isconsistent(variable,value,grid):
            ...
            arcresult = AC3(grid)
            checkPairs(grid)
            #result1 = setupGroupIntersection(grid)
            if not arcresult:
